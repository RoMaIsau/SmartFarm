...
public class ControladorMapaTest {
	...
	public void inicializar() {
		...
		this.servicioUbicacionesCentrales = mock(ServicioUbicacionesCentrales.class);
		...
		this.controladorMapa = new ControladorMapa(this.servicioUbicacion, this.servicioNotificacion,
				...
				this.servicioUbicacionesCentrales);
	}
	...
	public void dirigeAVistaMapaConUbicacionesYNotificaciones() {
		...
		List<Notificacion> notificacionesObtenidos = (List<Notificacion>) modelAndView.getModelMap()
				.get("notificaciones");
	...
	@Test
	public void enCambiarCoordenadasSiElROlEsVeterinarioRedirigeALaVistaLogin() {
		HttpServletRequest request = this.configurarRolLogueado("Veterinario");

		ModelAndView modelAndView = this.controladorMapa.irAModificarCoordenadas(request, new ModelMap());

		assertThat(modelAndView.getViewName()).isEqualTo("redirect:/login");
	}

	@Test
	public void enCambiarCoordenadasDeberiaCargarLasUbicacionesCentralesYLasNotificaciones() {
		...
		UbicacionesCentrales ubicacionesCentrales = new UbicacionesCentrales();
		ubicacionesCentrales.setId(1L);
		
		when(this.servicioUbicacionesCentrales.obtenerUbicacionesCentrales()).thenReturn(ubicacionesCentrales);
		
		TipoAnimal tipo1 = new TipoAnimal();
		tipo1.setId(1L);
		TipoAnimal tipo2 = new TipoAnimal();
		tipo2.setId(2L);
		
		List<TipoAnimal> tiposAnimales = new ArrayList<TipoAnimal>();
		tiposAnimales.add(tipo1);
		tiposAnimales.add(tipo2);
		
		when(this.servicioDeAnimales.obtenerTiposDeAnimales()).thenReturn(tiposAnimales);
		
		ModelAndView modelAndView = this.controladorMapa.irAModificarCoordenadas(request, new ModelMap());
		ModelMap modelo = modelAndView.getModelMap();
		
		verify(this.servicioDeAnimales).obtenerTiposDeAnimales();
		verify(this.servicioNotificacion).listarNotificaciones(eq(1L));
		verify(this.servicioUbicacionesCentrales).obtenerUbicacionesCentrales();
		
		List<Notificacion> notisObtenidas = (List<Notificacion>) modelo.get("notificaciones");
		List<TipoAnimal> tiposAnimalesObtenidos = (List<TipoAnimal>) modelo.get("tiposDeAnimales");
		UbicacionesCentrales ubiCentObtenido = (UbicacionesCentrales) modelo.get("ubicacionesCentrales");
		
		assertThat(notisObtenidas).hasSize(1);
		assertThat(tiposAnimalesObtenidos).isNotEmpty();
		assertNotNull(ubiCentObtenido);
	}
